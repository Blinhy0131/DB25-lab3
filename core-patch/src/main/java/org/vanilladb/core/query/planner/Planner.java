...
public class Planner {
	...
	public Plan createQueryPlan(String qry, Transaction tx) {
    	...
    	if (data.isExplain() == true) {
        	return new ExplainPlan(p); // 如果是EXPLAIN語句，包裝成ExplainPlan
    	}

    	return p; // 如果不是EXPLAIN語句，正常執行
	}
	public Plan createExplainplan(QueryData data, Transaction tx) {
		Plan p = qPlanner.createPlan(data, tx); // 
		return new ExplainPlan(p);
	}
	/**
